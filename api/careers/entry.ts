...
const jobPositionOptions = [
  { value: "designer", label: "空間デザイナー" },
  { value: "sales", label: "営業コンサルタント" },
  { value: "pm", label: "施工管理" },
] as const;

const jobPositionValues = jobPositionOptions.map(option => option.value) as [
  "designer",
  "sales",
  "pm",
];

const careersEntrySchema = z.object({
  name: z
    .string()
    .trim()
    .min(1, "氏名を入力してください。")
...
type CareersEntryPayload = z.infer<typeof careersEntrySchema>;

const jobPositionLabelMap = Object.fromEntries(
...
async function persistEntry(record: EntryRecord) {
  const storageDir = path.join(os.tmpdir(), "ls-hp-careers-entries");
  const storageFile = path.join(storageDir, "entries.jsonl");

  try {
    await fs.mkdir(storageDir, { recursive: true });
    await fs.appendFile(storageFile, `${JSON.stringify(record)}\n`, "utf8");
  } catch (error) {
    console.warn("Failed to persist entry in serverless runtime", error);
  }
}

function buildRecruiterMail(record: EntryRecord) {
  const positionLabel = jobPositionLabelMap[record.position];
  const motivationBlock = record.motivation || "未入力";
  ...
  return {
    ...
    html: `
      <div style="font-family: Arial, 'Hiragino Sans', 'Noto Sans JP', sans-serif; line-height: 1.7; color: #111827;">
        ...
        <p style="margin: 0 0 18px;">内容を確認のうえ、担当者よりご連絡いたします。</p>
